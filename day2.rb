...
def get_data line
  ...
  possible_positions = rule.split('-').map(&:to_i)
...
input.split("\n").filter { |line| line.length > 0}.each do |line|
  data = get_data line
  times_found = 0

  data[:password].split('').each_with_index do |letter, index|
    position = index + 1
    if letter == data[:letter]
      if data[:possible_positions].include?(position)
        times_found += 1
      end
    end
  end
  ...
  if times_found != 1
    bad_passes.push data[:password]
  end
end

p 1000 - bad_passes.size