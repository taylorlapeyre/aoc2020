...
FIELD_REQUIREMENTS = {
  ...
  'hgt' => ->(val) {
    is_cm = val.include?('cm')
    is_in = val.include?('in')
  ...
  'hcl' => ->(val) {
    val.size == 7 && val.match(/#[0-9a-f]{6}/)
  },
  'ecl' => ->(val) {
    ['amb', 'blu', 'brn', 'gry', 'grn', 'hzl', 'oth'].include?(val)
  },
  'pid' => ->(val) {
    val.size == 9 && val.match(/\d{9}/)
  }
}
...
def run input
  ...
  valid_passports = passports.filter do |passport|
    passport_fields = read_passport(passport)

    REQUIRED_FIELDS.all? { |field|
      !passport_fields[field].nil? && FIELD_REQUIREMENTS[field].(passport_fields[field])
    }
  end
